...
public class SecurityConfig {
    ...
    private static final String[] ROTAS_PUBLICAS_PAGINAS = {
            "/", "/login", "/registro", "/esqueci-senha", "/redefinir-senha",
            "/css/**", "/js/**", "/img/**", "/webjars/**", "/favicon.ico", "/favicon.png",
            "/manifest.json", "/sw.js"
    };
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(ROTAS_PUBLICAS_PAGINAS).permitAll()
                        .requestMatchers(ROTAS_PUBLICAS_API).permitAll()
                        .anyRequest().authenticated()
                )
                .exceptionHandling(eh -> eh
                        // Diferencia chamadas de API (401 JSON) de
                        // navegação de página (redirect para /login).
